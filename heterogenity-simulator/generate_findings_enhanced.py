...
def main():
    ...
    md_lines.append("")
    md_lines.append("**Key Correction**: The original research used L2 (center) + Cosine (extreme) vs L2 (homogeneous), which showed 0% disagreement because center voters used L2 in both cases. This research uses L1 (center) + Cosine (extreme) vs L1 (homogeneous) to reveal true heterogeneity effects.")
    md_lines.append("")
    md_lines.append("---")
    md_lines.append("")
    ...
    md_lines.append("")
    md_lines.append("**Critical Methodology Fix**: Experiments use L1-Cosine metric pair (both metrics differ) rather than L2-Cosine (where center metric matches homogeneous baseline).")
    md_lines.append("")
