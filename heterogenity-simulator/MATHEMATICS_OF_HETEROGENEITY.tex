...
\end{equation}
where, under the geometry $[-1,1]^d$, we use $d_{\max}^{(\mathrm{diag})}=2\sqrt{d}$ as the maximum possible Euclidean distance in the hypercube (the diagonal length). This ensures utilities are normalized to $[0,1]$ and are comparable across different voter positions.
