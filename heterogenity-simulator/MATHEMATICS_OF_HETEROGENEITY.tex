...
\section*{Spatial model and geometry domain (corrected)}
...
\bigskip
\noindent
\textbf{Merge note.} The remainder of this document follows the PDF structure \emph{Mathematics of Heterogeneous Distance Metrics in Spatial Voting: Simulator Specification and Methodology}, but applies the corrected geometry above whenever the PDF text assumes a $[0,1]^d$ domain.

% ---------------------------------------------------------------------------
% Main document: structure mirrors the PDF.
% ---------------------------------------------------------------------------
...
A critical consideration in experimental design is ensuring that the homogeneous baseline uses a metric different from the center metric in heterogeneous conditions. If both use the same metric (e.g., L2), center voters will have identical preferences in both conditions, leading to artificially low disagreement rates. The corrected methodology uses metric pairs where both metrics differ (e.g., L1--Cosine) to reveal true heterogeneity effects.
...
Voter utilities are computed from distances using a utility function. The primary function used in this research is the linear utility function:
\begin{equation}
  u_{v}(c)=\max\!\left(0,1-\frac{d_{v}(\mathbf{x}_{v},\mathbf{y}_{c})}{d_{\max}^{(\mathrm{diag})}}\right) \tag{12}
\end{equation}
where, under the corrected geometry $[-1,1]^d$, we use $d_{\max}^{(\mathrm{diag})}=2\sqrt{d}$ as the maximum possible Euclidean distance in the hypercube (the diagonal length). This ensures utilities are normalized to $[0,1]$ and are comparable across different voter positions.
...
\begin{itemize}
  ...
  \item $w^{\text{homo}}_{p}$ be the winner under homogeneous conditions (all voters use the baseline metric).
\end{itemize}
The disagreement rate is:
\begin{equation}
  D(m_{\text{center}},m_{\text{extreme}},\theta)=\frac{1}{N}\sum_{p=1}^{N}\mathbf{1}[w^{\text{het}}_{p}\ne w^{\text{homo}}_{p}]\times 100\% \tag{22}
\end{equation}

\subsection{Asymmetry Measure}
For a metric pair $(A,B)$, we define the asymmetry as:
\begin{equation}
  \Delta_{A,B}(\theta)=|D(A\rightarrow B,\theta)-D(B\rightarrow A,\theta)| \tag{23}
\end{equation}
...
\begin{itemize}
  ...
  \item Geometry method: uniform distribution in $[-1,1]^{d}$ (corrected geometry).
...
A critical validation step is ensuring that the homogeneous baseline uses a different metric from the center metric in heterogeneous conditions. If both use the same metric (e.g., L2), center voters will have identical preferences in both conditions, leading to artificially low disagreement rates. The corrected methodology uses metric pairs where both metrics differ (e.g., L1--Cosine) to reveal true heterogeneity effects.
