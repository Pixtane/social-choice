...
def generate_preferences(
    ...
    for i in range(n_profiles):
        ...
        utilities[i] = utility_computer.compute_utilities(
            distances,
            n_dim,
            voter_positions=spatial_profile.voter_positions[i],
            geometry=config.geometry,
        )

        # Normalize each voter's utilities: best candidate = 1, worst candidate = 0
        # For each voter, compute min and max across candidates
        u_min = np.min(utilities[i], axis=1, keepdims=True)  # (n_voters, 1)
        u_max = np.max(utilities[i], axis=1, keepdims=True)  # (n_voters, 1)
        u_range = u_max - u_min  # (n_voters, 1)

        # Normalize: (u - u_min) / (u_max - u_min)
        # Handle edge case where all utilities are the same (u_range ≈ 0)
        # In that case, set all utilities to 1.0 (since all are equally best)
        mask = u_range > 1e-12
        utilities_normalized = np.where(
            mask,
            (utilities[i] - u_min) / u_range,
            np.ones_like(utilities[i])  # All utilities are the same, set all to 1.0
        )
        utilities[i] = utilities_normalized
