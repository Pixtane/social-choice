...
class VotingRuleEngine:
    ...
    def irv(self, rankings: np.ndarray) -> VotingResult:
        ...
        for round_num in range(n_candidates - 1):
            ...
            if np.max(vote_counts) > active_total / 2:
                winner = self._tiebreak_ext(vote_counts, maximize=True, active_mask=active)
                break

            # Eliminate candidate with fewest votes
            eliminated = self._tiebreak_ext(vote_counts, maximize=False, active_mask=active)
            active[eliminated] = False
            elimination_order.append(eliminated)
        else:
            # Last candidate standing (exactly one should remain after n-1 eliminations)
    ...
    def coombs(self, rankings: np.ndarray) -> VotingResult:
        ...
        for round_num in range(n_candidates - 1):
            ...
            eliminated = self._tiebreak_ext(last_counts, maximize=True, active_mask=active)
            active[eliminated] = False
            elimination_order.append(eliminated)
        else:
            # Last candidate standing (exactly one should remain after n-1 eliminations)
    ...
    def ranked_pairs(self, rankings: np.ndarray) -> VotingResult:
        ...
        winners = [c for c in range(n_candidates) if c not in losers]

        if len(winners) == 1:
            winner = winners[0]
        elif len(winners) > 1:
            # Multiple winners (shouldn't happen in standard ranked pairs, but handle with tie-breaking)
            winner_scores = np.zeros(n_candidates)
            winner_scores[winners] = 1.0
            winner = self._tiebreak_ext(winner_scores, maximize=True)
        else:
    ...
    def bucklin(self, rankings: np.ndarray) -> VotingResult:
        ...
        for round_num in range(n_candidates):
            ...
            if np.max(scores) > majority:
                winner = self._tiebreak_ext(scores, maximize=True)
                return VotingResult(
                    winner=winner,
                    scores=scores,
    ...
    def nanson(self, rankings: np.ndarray) -> VotingResult:
        ...
        # Select winner from remaining active candidate (should be exactly one)
        winner = np.argmax(active)
        scores = np.zeros(n_candidates)
        scores[winner] = 1
    ...
    def baldwin(self, rankings: np.ndarray) -> VotingResult:
        ...
        while np.sum(active) > 1:
            ...
            eliminated = self._tiebreak_ext(scores, maximize=False, active_mask=active)
            active[eliminated] = False
            elimination_order.append(eliminated)
    ...
    def _tiebreak(self, scores: np.ndarray) -> int:
        """
        Break ties using configured method (for maximization/winner selection).
        ...
        Returns:
            Winner index
        """
        return self._tiebreak_ext(scores, maximize=True)

    def _tiebreak_ext(self, scores: np.ndarray, maximize: bool = True, active_mask: Optional[np.ndarray] = None) -> int:
        """
        Break ties using configured method, supporting both maximization and minimization.

        This method respects the configured tiebreak_method for all voting rules,
        including iterative elimination methods like IRV.

        Args:
            scores: Score array
            maximize: If True, select candidate with maximum score (winner selection).
                      If False, select candidate with minimum score (elimination).
            active_mask: Optional boolean mask indicating which candidates are active.
                        If provided, only considers active candidates.

        Returns:
            Selected candidate index
        """
        if active_mask is not None:
            # Mask out inactive candidates
            masked_scores = np.where(active_mask, scores, np.inf if maximize else -np.inf)
        else:
            masked_scores = scores

        if maximize:
            target_value = np.max(masked_scores)
            tied = np.where(masked_scores == target_value)[0]
        else:
            target_value = np.min(masked_scores)
            tied = np.where(masked_scores == target_value)[0]
