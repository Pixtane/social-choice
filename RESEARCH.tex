...
\begin{itemize}
    ...
    \item \textbf{Random Seed}: Fixed seed (42) for reproducibility.
\end{itemize}
...
A fundamental challenge in spatial voting is interpersonal utility comparison. We solve this through per-voter normalization to the $[0,1]$ range using a \textit{linear} utility function (not Gaussian). All experiments in this study use linear utility consistently.

\begin{definition}[Linear Utility with Best-Worst Scaling]
    For voter $v$ with position $\mathbf{x}_v$ and candidate $c$ with position $\mathbf{y}_c$:
    \begin{align}
        u_{v,c} = 1 - \frac{d(\mathbf{x}_v, \mathbf{y}_c)}{d_{\max,v}}
    \end{align}
    where $d_{\max,v}$ is the maximum possible distance from voter $v$ to any point in the hypercube, computed as the distance to the "opposite corner":
    \begin{align}
        d_{\max,v} = d(\mathbf{x}_v, \text{opposite}(\mathbf{x}_v))
    \end{align}
\end{definition}

This normalization ensures:
\begin{itemize}
    \item $u_{v,c} \in [0,1]$ for all voters and candidates
    \item Each voter's best candidate (closest) has utility 1
    \item Each voter's worst candidate (farthest) has utility 0
    \item Utilities are comparable across voters despite different distance metrics
    \item \textbf{Linear relationship}: Utility decreases linearly with distance (not exponentially as in Gaussian models)
\end{itemize}

The opposite corner is defined coordinate-wise: for each dimension $i$, we choose the bound ($-1$ or $+1$) that is farthest from $x_{v,i}$.
...
\textbf{Note on Disagreement Decomposition}: This paper reports decomposed disagreement rates using the methodology defined in the simulation documentation \cite{methodology}. Each disagreement is classified as either:
\begin{itemize}
    \item \textbf{Strong Disagreement}: Heterogeneous winner differs from \textit{both} center-metric and extreme-metric homogeneous baselines
    \item \textbf{Extreme-Aligned Disagreement}: Heterogeneous winner matches extreme-metric baseline but differs from center-metric baseline
\end{itemize}

The total disagreement rate is the sum of these two components. This decomposition reveals whether heterogeneity introduces genuinely novel outcomes (strong disagreement) or merely shifts outcomes toward the extreme-metric baseline (extreme-aligned disagreement).
...
Our empirical results demonstrate a striking convergence phenomenon: as dimension increases, voting rules yield increasingly similar outcomes regardless of metric heterogeneity.

\subsubsection{Empirical Evidence}

Table~\ref{tab:dimensional_convergence} presents disagreement rates (percentage of profiles where heterogeneous and center-metric homogeneous winners differ) for L2-Cosine metric pairs across dimensions 1--10:
...
Table~\ref{tab:metric_pairs} presents decomposed disagreement rates for all metric pairs at dimension 2:
...
\begin{table}[h]
    ...
    \begin{tabular}{lccc}
        ...
        L1 $\rightarrow$ L2            & 15.5\%    & 17.5\%          & 21.0\% \\
        L2 $\rightarrow$ L1            & 15.5\%    & 17.5\%          & 21.0\% \\
        Chebyshev $\rightarrow$ L1     & 14.0\%    & 17.5\%          & 21.5\% \\
        Chebyshev $\rightarrow$ L2     & 15.5\%    & 17.5\%          & 22.0\% \\
        Chebyshev $\rightarrow$ Cosine & 15.5\%    & 14.0\%          & 22.0\% \\
        \bottomrule
    \end{tabular}
\end{table}

This reveals a \textbf{dramatic asymmetry}: Cosine-to-Minkowski pairs show vastly different disagreement than Minkowski-to-Cosine pairs, and this effect is \textit{rule-dependent}:

\begin{itemize}
    \item \textbf{Borda}: Shows extreme directional sensitivity. Cosine$\rightarrow$L2 has 60.0\% disagreement vs L2$\rightarrow$Cosine's 8.5\%---a 51.5pp difference. This suggests Borda is highly sensitive to which metric is assigned to center vs extreme voters.

    \item \textbf{IRV}: Shows \textit{reverse} asymmetry. L2$\rightarrow$Cosine has 12.5\% disagreement vs Cosine$\rightarrow$L2's 4.5\%.

    \item \textbf{Plurality}: Relatively symmetric (14--16\% disagreement for both directions).
\end{itemize}

The Borda asymmetry is particularly striking: when Cosine is the center metric, outcomes differ from the L2-baseline 60\% of the time, but when L2 is the center metric, disagreement drops to 8.5\%. This suggests that the assignment of metrics to voter groups matters as much as the choice of metrics themselves.
...
\begin{itemize}
    ...
    \item For Cosine$\rightarrow$L2 Borda, strong disagreement is only 19.2\% (with 48.5\% extreme-aligned), suggesting the high total disagreement (60\%) is mostly due to alignment with the Cosine-homogeneous baseline rather than genuinely novel outcomes.
\end{itemize}

This decomposition clarifies that \textit{what} heterogeneity does depends critically on both the voting rule and the direction of metric assignment.
...
The convergence can be understood through measure concentration: as dimension increases, the effective configuration space shrinks. Most voters cluster near the effective radius ($\approx 0.577$), making center-extreme distinctions less meaningful. Additionally, distance distributions become more uniform, reducing the impact of metric choice on utility rankings.
...
\begin{itemize}
    \item Total disagreement: 60.0\% (vs 8.5\% for L2$\rightarrow$Cosine)
    \item Strong disagreement: 11.5\% (19.2\% of total)
    \item Extreme-aligned disagreement: 48.5\% (80.8\% of total)
    \item This indicates most disagreements represent alignment with the L2-homogeneous baseline, not genuinely novel outcomes
\end{itemize}

This "Borda-Cosine Interaction" suggests that Cosine distance, when assigned to centrist voters under Borda count, creates a dramatically different strategic landscape. The directional nature of Cosine distance---focusing on ideological alignment rather than policy distance---may interact with Borda's preference summation in ways that amplify heterogeneity effects.

\subsubsection{L1/L2 Instability}

In contrast, L1 and L2 metrics show higher sensitivity to heterogeneity. For example, L1-L2 pairs at dimension 2 show:
\begin{itemize}
    \item Disagreement (vs center baseline): 15.5\% (Plurality), 17.5\% (Borda), 21.0\% (IRV)
    \item Cycle rate reduction: -2.0\% (heterogeneous has fewer cycles than center-metric baseline)
    \item Condorcet efficiency improvement: +9.2\% (Plurality), +3.1\% (Borda), +1.6\% (IRV)
\end{itemize}

The negative cycle rate delta indicates that metric heterogeneity can actually \textit{reduce} Condorcet paradoxes, potentially by breaking symmetry in preference profiles.
...
Key observations:
\begin{enumerate}
    \item \textbf{Borda consistently outperforms}: VSE $\geq 0.95$ across all dimensions, approaching 1.0 in high dimensions.
    \item \textbf{IRV shows strong performance}: VSE $\geq 0.92$, with improvement in higher dimensions.
    \item \textbf{Plurality is weakest}: VSE ranges 0.75--0.96, but improves significantly with dimension.
    \item \textbf{Dimensional improvement}: All rules show increasing VSE as dimension increases, suggesting that high-dimensional spaces naturally favor better social choices.
\end{enumerate}

\subsubsection{Metric Heterogeneity Effects on VSE}

For L2-Cosine pairs, VSE differences between heterogeneous and homogeneous conditions are zero across all dimensions and rules. This reinforces the convergence result: metric heterogeneity has no effect on social welfare when using this specific metric pair.

However, for other metric pairs, VSE differences can be substantial. For example, Cosine-L1 at dimension 2 shows VSE improvements of 5--9\% for heterogeneous conditions, suggesting that strategic metric assignment can improve social outcomes.
...
\begin{enumerate}
    ...
    \item \textbf{Decomposition reveals mechanisms}: Strong disagreement (het differs from both baselines) ranges from 10--93\% of total disagreement, depending on rule and configuration. Borda shows highest strong disagreement proportion, suggesting it introduces genuinely novel outcomes.
...
\begin{enumerate}
    \item \textbf{Theoretical understanding of Borda-Cosine interaction}: Why does Cosine-center Borda produce 7x higher disagreement than L2-center? Can we derive analytical bounds on this asymmetry?
